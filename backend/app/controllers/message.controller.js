...
exports.create = (req, res) => {
  ...
  if (!req.body.messageName || !req.body.messageEmail || !req.body.messageContent) {
  ...
  MessageModel.findById(req.params.id, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        ...
        res.status(500).send({ message: "Error retrieving message with id " + req.params.id });
      }
    } else {
      res.send(data);
    }
...
exports.update = (req, res) => {
  ...
  MessageModel.updateById(req.params.id, new MessageModel(req.body), (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({ message: `Message not found with id ${req.params.id}` });
      } else {
        res.status(500).send({ message: "Error updating message with id " + req.params.id });
      }
    } else {
      res.send(data);
    }
  ...
  MessageModel.removeOne(req.params.id, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        ...
        res.status(500).send({ message: "Could not delete the message with id " + req.params.id });
      }
    } else {
